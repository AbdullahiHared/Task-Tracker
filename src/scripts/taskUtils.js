...
export function addTaskToArray(dataName, userTask) {
  taskData.forEach((taskCategory) => {
    if (taskCategory.name === dataName) {
      const existingIndex = taskCategory.data.findIndex(
        (task) =>
          task.title === userTask.title &&
          task.date === userTask.date &&
  ...
  const allTasksIndex = allTasksData.findIndex(
    (task) =>
      task.title === userTask.title &&
      task.date === userTask.date &&
      task.description === userTask.description,
...
export function removeTaskFromArray(dataName, taskToRemove) {
  taskData.forEach((taskCategory) => {
    if (taskCategory.name === dataName) {
      const index = taskCategory.data.findIndex(
        (task) =>
...
export function getTasksByCategory(categoryName) {
  return (
    taskData.find((category) => category.name === categoryName)?.data || []
  );
}
...
export function createTaskModifiers(task, index, category) {
  ...
  starTask.addEventListener("click", () => {
    ...
    displayTaskAdder(currentCategory); // Re-display tasks in the current category
  });
